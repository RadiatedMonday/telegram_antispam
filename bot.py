...
async def send_log(message: types.Message, type: str):
    ...
    reason = "зафиксирован спам" if type == "classify" else "сообщение переслано от канала или бота" if type == "forwarded_from_" else "сообщение подержит в себе 3 или более эмодзи подряд"
...
async def markup(message: types.Message, withFalse: bool, withBan: bool):
    ...
    builder.add(InlineKeyboardButton(text="❗️Ложное", callback_data=f"false:{message.message_id}")) if withFalse else None
    builder.add(InlineKeyboardButton(text="⛔️ Исключить", callback_data=f"ban:{message.message_id}")) if withBan else None
